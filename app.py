...
def extract_text_preserve_breaks(node: Tag | NavigableString, annotate_links: bool) -> str:
    ...
    return "".join(parts)
def find_first_h1_element(body: Tag) -> Tag | None:
    """Find the first <h1> element in the body."""
    return body.find("h1")
def get_elements_from_h1_onwards(body: Tag, start_from_h1: bool) -> list[Tag]:
    """
    Get all elements that should be processed, either all elements or only those
    from the first H1 onwards.
    """
    if not start_from_h1:
        # Return all elements as before
        return list(body.descendants)
    
    first_h1 = find_first_h1_element(body)
    if not first_h1:
        # No H1 found, return empty list (or all elements - you can decide)
        return list(body.descendants)
    
    # Find all elements that come at or after the first H1
    elements_to_process = []
    h1_found = False
    ...
    def collect_elements(element):
        ...
        if isinstance(element, Tag):
            ...
            if h1_found:
                ...
                # Also collect all descendants of this element
                for desc in element.descendants:
                    elements_to_process.append(desc)
        elif h1_found and isinstance(element, NavigableString):
            elements_to_process.append(element)
    
    # Walk through all elements in document order
    for element in body.descendants:
        if isinstance(element, Tag) and element == first_h1:
            h1_found = True
        if h1_found:
            elements_to_process.append(element)
    
    return elements_to_process
def extract_signposted_lines_from_body(body: Tag, annotate_links: bool, start_from_h1: bool = False) -> list[str]:
    """
    Emit ONLY:
      - <h1> … <h6> lines
      - <p> lines
    ...
    If start_from_h1 is True, only process content from the first H1 onwards.
    """
    lines: list[str] = []
    ...
    if start_from_h1:
        # Only process content from first H1 onwards
        first_h1 = find_first_h1_element(body)
        if not first_h1:
            # No H1 found, return empty list
            return []
        
        # Find all top-level elements that come at or after the first H1
        h1_found = False
...
def process_url(
    ...
    lines = extract_signposted_lines_from_body(body, annotate_links=annotate_links, start_from_h1=start_from_h1)
    # meta (this is unaffected by start_from_h1 option)
...
with st.sidebar:
    ...
    start_from_h1 = st.toggle("Start content extraction from first H1", value=False)
    st.caption("When enabled, ignores all content before the first <h1> tag (useful for skipping navigation menus)")
...
with tab1:
    ...
    if do_preview or do_doc:
        if not tpl_file and do_doc:
            ...
            try:
                meta, lines = process_url(url, exclude_selectors, annotate_links=annotate_links, start_from_h1=start_from_h1)
        ...
        if not tpl_file:
            ...
            if not rows:
                ...
                for i, row in enumerate(rows, 1):
                    u = row["url"].strip()
                    try:
                        meta, lines = process_url(u, exclude_selectors, annotate_links=annotate_links, start_from_h1=start_from_h1)
